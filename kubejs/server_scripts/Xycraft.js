ServerEvents.recipes(nadien => {
    ...
    function extractor({output, target, adjacent, catalyst, directions, ticks, waterlogged}) {
        ...
        nadien.custom(recipe).id(`nadien_tweaks:xycraft/extractor/${target.block.split(':').pop()}_to_${output.item.split(":").pop()}`);
    }
    ...
    //extractor
    nadien.remove({output: 'xycraft_machines:extractor' })
    nadien.shaped('xycraft_machines:extractor', [
      'ABA',
      'CDC',
      'EFE'
    ],
      {
        ...
        A: 'enderio_endergy:stellar_capacitor',
        E: 'oritech:duratium_ingot'
    }).id('nadien_tweaks:xycraft_extractor')
})